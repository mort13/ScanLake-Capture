...
function toBigInt64(nums: number[]): BigInt64Array {
  return BigInt64Array.from(nums.map(n => BigInt(Math.round(n))))
}
...
export function buildScansParquet(scans: Scan[], profile: UserProfile): ArrayBuffer {
  return parquetWriteBuffer({
    columnData: [
      ...
      { name: 'deposit_conf',     data: Float64Array.from(scans.map(() => 1.0)),         type: 'DOUBLE' },
      { name: 'mass',             data: toBigInt64(scans.map(s => s.mass)),              type: 'INT64' },
      { name: 'mass_conf',        data: Float64Array.from(scans.map(() => 1.0)),         type: 'DOUBLE' },
      { name: 'resistance',       data: toBigInt64(scans.map(s => s.resistance)),        type: 'INT64' },
      { name: 'resistance_conf',  data: Float64Array.from(scans.map(() => 1.0)),         type: 'DOUBLE' },
      { name: 'instability',      data: Float64Array.from(scans.map(s => s.instability)), type: 'DOUBLE' },
      { name: 'instability_conf', data: Float64Array.from(scans.map(() => 1.0)),         type: 'DOUBLE' },
      { name: 'volume',           data: Float64Array.from(scans.map(s => s.volume)),     type: 'DOUBLE' },
      { name: 'volume_conf',      data: Float64Array.from(scans.map(() => 1.0)),         type: 'DOUBLE' },
    ],
  })
}
...
export function buildCompositionsParquet(scans: Scan[], materials: Material[]): ArrayBuffer {
  ...
  return parquetWriteBuffer({
    columnData: [
      ...
      { name: 'mat_index',       data: toBigInt64(enriched.map(m => m.matIndex)),            type: 'INT64' },
      { name: 'type',            data: enriched.map(m => m.type),                            type: 'STRING' },
      { name: 'amount',          data: Float64Array.from(enriched.map(m => m.amount)),       type: 'DOUBLE' },
      { name: 'quality',         data: toBigInt64(enriched.map(m => m.quality)),             type: 'INT64' },
      { name: 'material_volume', data: Float64Array.from(enriched.map(m => m.materialVolume)), type: 'DOUBLE' },
      { name: 'min_confidence',  data: Float64Array.from(enriched.map(() => 1.0)),           type: 'DOUBLE' },
    ],
  })
}
